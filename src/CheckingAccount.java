public class CheckingAccount {
    // we nee to count the number of accounts in existence (static)
    // this is private so that the only code that can see/change this lives on this particular class
    private static int numberOfAccounts = 0;

    // private means available only to other code within the exact same class
    private String name;
    ...
    public CheckingAccount (String name, String accountType){
        ...
        accountNumber = 1000L + numberOfAccounts;
    }
    // all getters are public so we can provide public access (if we need) to private/protected items
    ...
    public double withdraw(double amount){
        if (amount<= balance) {
            ...
            //update balance to display the difference
            balance -= amount;
